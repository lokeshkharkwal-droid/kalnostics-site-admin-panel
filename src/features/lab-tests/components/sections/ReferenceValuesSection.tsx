...
export function ReferenceValuesSection({ data, setData }: { data: LabTest; setData: Dispatch<SetStateAction<LabTest>> }) {
  ...
  const empty = (): ReferenceValueItem => ({
    id: `rv-${Date.now()}`, parameter: '', method: '', gender: 'All',
    ageFrom: '0', ageFromUnit: 'Years', ageTo: '999', ageToUnit: 'Years',
    displayRange: '', abnormalFlagLogic: 'Bold and Red', isDefault: false,
  })
  const [form, setForm] = useState<ReferenceValueItem>(empty())

  const handleSave = () => {
    setData(prev => editItem
      ? { ...prev, referenceValues: prev.referenceValues.map(r => r.id === editItem.id ? form : r) }
      : { ...prev, referenceValues: [...prev.referenceValues, form] })
    setFormOpen(false)
  }
  const makeDefault = (id: string) => setData(prev => ({ ...prev, referenceValues: prev.referenceValues.map(r => ({ ...r, isDefault: r.id === id })) }))
  const deleteValue = (id: string) => setData(prev => ({ ...prev, referenceValues: prev.referenceValues.filter(r => r.id !== id) }))

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium text-notion-sub">{data.referenceValues.length} reference value(s)</span>
        <Button size="sm" onClick={() => { setForm(empty()); setEditItem(null); setFormOpen(true) }}><PlusIcon className="h-3.5 w-3.5" />Add Value</Button>
      </div>

      {data.referenceValues.map(r => (
        <div key={r.id} className={cn('flex items-start justify-between gap-3 rounded-lg border p-3', r.isDefault ? 'border-blue-200 bg-blue-50/40' : 'border-notion-line bg-notion-panel')}>
          <div className="grid flex-1 grid-cols-4 gap-2 text-xs text-notion-sub">
            ...
            <span><strong>Age:</strong> {r.ageFrom}–{r.ageTo}</span>
          ...
          <div className="flex shrink-0 items-center gap-1">
            <button type="button" title="Edit reference value" onClick={() => { setForm({ ...r }); setEditItem(r); setFormOpen(true) }} className="rounded p-1.5 text-notion-sub hover:bg-notion-hover"><PencilIcon className="h-3.5 w-3.5" /></button>
            {!r.isDefault && <Button size="sm" variant="secondary" onClick={() => makeDefault(r.id)}>Set Default</Button>}
            <button type="button" title="Delete reference value" onClick={() => deleteValue(r.id)} className="rounded p-1.5 text-notion-red hover:bg-notion-hover"><TrashIcon className="h-3.5 w-3.5" /></button>
          </div>
        </div>
      ))}
      {data.referenceValues.length === 0 && (
        <div className="rounded-lg border border-dashed border-notion-line2 py-8 text-center text-sm text-notion-faint">No reference values added yet</div>
      )}

      {formOpen && (
        <Modal
          title={editItem ? 'Edit Reference Value' : 'Add Reference Value'}
          size="lg"
